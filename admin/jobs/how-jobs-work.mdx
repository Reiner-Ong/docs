...
A <Tooltip cta="job" href="/start/glossary">job</Tooltip> is one visit to one
building. Every time a technician goes on-site, a job records it — and when the
job is closed, what the technician wrote becomes the permanent history of those
lifts.
...
<Steps>
  <Step title="Open">
    The job exists, but nobody is assigned and no time is set. Most jobs skip
    straight past this — the **Schedule Job** button creates and schedules in one
    action. You mostly see **Open** on jobs that arrived another way, such as a
    converted quote.
  </Step>
  ...
  <Step title="Work Done">
    The technician has finished on-site and submitted the work. The
    <Tooltip cta="records" href="/start/glossary">records</Tooltip> exist, each
    signed by the technician who did that lift. The job is now waiting for the
    building side to sign off.
  </Step>
  ...
  <Step title="Closed">
    An admin has reviewed and closed it. The PDFs are generated, linked issues are
    closed, and a draft invoice is raised. This is the end of the line.
  </Step>
</Steps>
...
| Anything → Cancelled | Admin, owner, or technician | Dashboard |
...
<CardGroup cols={2}>
  ...
  <Card title="A converted quote" icon="file-invoice" href="/admin/invoices-and-payments">
    Arrives as an **Open** job with its required parts already listed.
  </Card>
</CardGroup>
